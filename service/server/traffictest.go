        ...
        for _ = range sendDlMsg.C {
            for _, context := range trafficTestList {
                if context.Parameters != nil {
                    ...
                    } // UL test running, check for timeout (don't worry about the DL as the UTM will timeout
                    ...
                    switch utmMsg := value.Message.(type) {
                        ...
                        case *ModeSetReqDlMsg:
                        {
                            ...
                            if utmMsg.Mode != MODE_TRAFFIC_TEST {
                                context.DlState = TRAFFIC_TEST_NOT_RUNNING;
                                context.UlState = TRAFFIC_TEST_NOT_RUNNING;
                                context.DlTimeStopped = time.Now().UTC()
                                context.UlTimeStopped = time.Now().UTC()
                        ...
                        case *TrafficTestModeReportIndUlMsg:
                        {
                            globals.Dbg.PrintfTrace("%s [traffic_test] --> received report from %s, assessing DL state.\n",
                                 globals.LogTag, value.DeviceUuid)
                            // Assess whether we're donen in the downlink direction from the report
                        ...
                        case *BadTrafficTestModeRuleBreakerUlDatagram:
                        {
                            if context.UlState == TRAFFIC_TEST_RUNNING {
                                ...
                                context.UlFill++
                                context.TimeLastUl = time.Now().UTC()
                                globals.Dbg.PrintfTrace("%s [traffic_test] --> received %d BAD UL traffic test mode datagram(s) %d from  %s, incremented expected fill to %d.\n",
                                    globals.LogTag, context.UlDatagramsBad, value.DeviceUuid, context.UlFill)
                            }
                        }
                        case *OutOfSequenceTrafficTestModeRuleBreakerUlDatagram:
                        {
                            if context.UlState == TRAFFIC_TEST_RUNNING {
                                ...
                                context.TimeLastUl = time.Now().UTC()
                                globals.Dbg.PrintfTrace("%s [traffic_test] --> received %d OOS UL traffic test mode datagram(s) %d from  %s, incremented expected fill to %d.\n",
                                    globals.LogTag, context.UlDatagramsOOS, value.DeviceUuid, context.UlFill)
                            }
                        }
                        default:
                        {
